...
# 20151201
startbefore1month=`date -d "-1 month ${start}" +%Y%m%d`
start1month=`date -d "1 month ${start}" +%Y%m%d`




tomorrow=`date -d "1 day ${start}" +%Y%m%d`

yesterday_of_start=`date -d "-1 day ${start}" +%Y%m%d`
...
minusDay1=`date -d "-1 day 20160101" +%Y-%m-%d`
